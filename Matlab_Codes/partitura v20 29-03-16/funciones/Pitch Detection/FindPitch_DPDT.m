function [frec_Ffund,numnota] = FindPitch_DPDT(in,fs,MIN_FREC,base, normalizacion, piso_detec, plot_detec)
%% [frec_Ffund,numnota] = FindPitch_DPDT(in,fs,MIN_FREC,base,
%                                           normalizacion, do_detec)
%
%   Halla la frecuencia fundamental del bloque de entrada
%
%   [frec_Ffund]:   Nota detectada en letras
%   [numnota]:      Nota detectada en número
%   in:             Bloque de entrada. Usar 2048 muestras
%   fs:             Frecuencia de muestreo;
%   MIN_FREC:       Frecuencia mínima a detectar.
%                   En caso de la guitarra, 80Hz.
%   piso_detec:     Coeficiente para el piso de ruido
%   plot_detec:       'true' plotea niveles de energía,
%                   'false' no plotea nada
    ...
    piso_de_ruido = piso_ruido(PSD, PSD_x, piso_detec);
...
%    [max_rel_y,max_rel_x] = max_relativos(PSD,piso_de_ruido);   % Vector de indices obtenidos de maximos relativos
    ...
    if (plot_detec == true) && (strcmp(frec_Ffund,'none') == 0)
        figure;
        stem(PSD_x, PSD);
        hold on;
